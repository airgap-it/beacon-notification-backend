...
enum StorageKey {
  MATRIX_PRESERVED_STATE = 'beacon:matrix-preserved-state',
}
...
export class MatrixClientStore {
   ...
   */
  private waitReadyPromise: Promise<void> = new Promise<void>(
   ...
   */
  private prepareData(
    toStore: Partial<MatrixStateStore>,
  ): Partial<MatrixStateStore> {
  ...
  private updateStorage(stateUpdate: Partial<MatrixStateUpdate>): void {
    const updatedCachedFields = Object.entries(stateUpdate).filter(
      ([key, value]) =>
        PRESERVED_FIELDS.includes(key as keyof MatrixStateUpdate) &&
        Boolean(value),
    );
    ...
    if (updatedCachedFields.length > 0) {
      ...
      this.storage.set(
        ...
        this.prepareData(filteredState),
      );
    }
  }
  ...
  private mergeRooms(
    ...
    const newRooms: MatrixRoom[] = Array.isArray(_newRooms)
      ? _newRooms
      : Object.values(_newRooms);
