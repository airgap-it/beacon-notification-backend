...
export class MatrixClientEventEmitter extends EventEmitter {
  ...
  > = new Map([
    [
      MatrixClientEventType.INVITE,
      () => [this.isInvite, this.emitInvite.bind(this)],
    ],
    [
      MatrixClientEventType.MESSAGE,
  ...
  private isMessage(
    ...
    return stateChange.rooms
      ? stateChange.rooms.some((room) => room.messages.length > 0)
      : false;
  }
  ...
  private emitMessage(
    ...
    stateChange.rooms
      ...
      .map((room) =>
        room.messages.map(
          (message) => [room.id, message] as [string, MatrixMessage<any>],
        ),
      )
