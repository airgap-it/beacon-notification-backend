...
export class MatrixClient {
   ...
   */
  public async inviteToRooms(
    user: string,
    ...roomsOrIds: string[] | MatrixRoom[]
  ): Promise<void> {
    ...
    const pollSync = async (
      ...
      try {
        ...
        if (continueSyncing) {
          ...
          reject(
            new Error(`Max polling retries exeeded: ${MAX_POLLING_RETRIES}`),
          );
        }
      }
    };
  ...
  private async requiresAuthorization<T>(
    ...
    if (!storedToken) {
      return Promise.reject(
        `${name} requires authorization but no access token has been provided.`,
      );
    }
